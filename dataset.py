...
from data_utils import load_recording
...
class SleepWindowDataset(Dataset):
    ...
    def __init__(self, files: List[Path], scaler: StandardScaler) -> None:
        ...
        for path in files:
            ...
            features = scaler.transform(df[FEATURE_COLS].values)  # (N, 7) float64→normalised
            ...
            for centre in range(CENTER_IDX, n_epochs - CENTER_IDX):
                window = features[centre - CENTER_IDX : centre + CENTER_IDX + 1]  # (31, 7)
        ...
        self._windows = np.stack(windows_list, axis=0).astype(np.float32)  # (N, 31, 7)
        self._labels  = np.array(labels_list, dtype=np.int64)              # (N,)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        x = torch.from_numpy(self._windows[idx])     # shares memory — zero-copy
